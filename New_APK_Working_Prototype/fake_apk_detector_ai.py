...
def scan_apk(apk_data):
    ...
    # Calculate risk score with AI features
    risk_score, risk_factors = calculate_risk_score(apk_data, official_comparison)
...
def main():
    ...
    if apk_data:
        ...
        with col2:
            ...
            keywords, keyword_risk = detect_fraud_keywords(app_name)
            if keywords:
                st.subheader("Fraud Keywords Detected")
                for keyword_info in keywords:
                    st.write(f"⚠️ '{keyword_info['keyword']}' ({keyword_info['language']}) - Risk: +{keyword_info['risk_score']}")
